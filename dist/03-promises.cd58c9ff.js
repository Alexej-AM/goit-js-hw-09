...
function onFromSubmit(event) {
    ...
    let delay1 = firstDelay;
    for(position = 1; position <= amount; position += 1){
        createPromise(position, delay1).then(({ position , delay  })=>(0, _notiflixNotifyAio.Notify).success(`✅ Fulfilled promise ${position} in ${delay}ms`)).catch(({ position , delay  })=>(0, _notiflixNotifyAio.Notify).failure(`❌ Rejected promise ${position} in ${delay}ms`));
        delay1 += step;
    }
}
